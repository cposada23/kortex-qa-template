...
| [install-hooks.mjs](install-hooks.mjs) | Install / refresh the Git pre-commit hook (runs `validate.mjs` + `build-index.mjs --check` + `validate-links.mjs`). Idempotent; refuses to clobber a non-Kortex hook. |
