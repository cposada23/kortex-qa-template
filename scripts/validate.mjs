...
// Brain-local exemptions (v2.3.0): a brain can exempt directories of
// AS-IS imported source material (client docs in other languages,
// pre-schema files) via brain.config.json:
//   "validation_exempt_dirs": ["client-context"]
// The setting is DATA, so it survives template upgrades that replace
// scripts/. Entries are directory NAMES (not paths), matching how
// SKIP_DIRS is checked during the walk.
try {
  const cfg = JSON.parse(readFileSync(path.join(REPO_ROOT, 'brain.config.json'), 'utf8'));
  if (Array.isArray(cfg.validation_exempt_dirs)) {
    for (const d of cfg.validation_exempt_dirs) {
      if (typeof d === 'string' && d.trim()) SKIP_DIRS.add(d.trim());
    }
  }
} catch {
  // No/unparseable brain.config.json (pre-init checkout) — defaults only.
}
