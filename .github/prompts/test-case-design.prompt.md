...
4. The target location:
   - **Story-local:**
     `teams/<active>/stories/<TICKET-KEY>-<slug>/test-cases/<tc-slug>.md`
     when this test is unique to this story.
   - **Library:** `teams/<active>/test-cases/library/<area>/<tc-slug>.md`
     when it's promotable (reusable across stories).

## Process

1. Confirm with the engineer: story-local or library?
2. If library, assign the next free `TC-<AREA>-<NNN>` ID by
   checking the existing IDs in
   `teams/<active>/test-cases/library/<area>/`. If story-local,
   use `TC-<TICKET-KEY>-<NN>` local sequence.
...
- [ ] `area:` is a directory name under `teams/<active>/test-cases/library/`
...
Output file path: `teams/<active>/test-cases/library/search/tc-search-005-filter-by-date-range-happy.md`
