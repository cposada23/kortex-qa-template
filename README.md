...
generates the Copilot prompts, Claude skills, the AGENTS.md skills
index (Gemini shim) and the mcp.json copies — edit canonical only,
pre-commit blocks drift. (2) `AGENTS.md` rewritten compact (≤10 KB,
commands + boundaries + generated index); per-agent wrappers
(`CLAUDE.md` = `@AGENTS.md` import). (3) Traceability schema v2:
stories use stable `### AC-n:` headings; TCs gain `covers_ac`,
`external_ids`, `last_run`/`last_result`; per-team
**coverage-matrix.md** (story × AC → TC) generated by build-index;
`validate-links` blocks phantom covers_ac; new
`validate-automation.mjs` (TC → spec traceability, 5th hook check);
forbidden-phrases gate + automation freshness warnings. (4) New
machine state `brain.config.json` + `doctor.mjs` day-1 preflight
(corporate proxy/TLS diagnosis) wired into init. (5) New skills:
`week-one` (5-day discovery, reentrant), `sut-map` (+ 
`knowledge/sut-map/` zone), `automation-bootstrap` (kortex-test
init bridge) and `automation-sync` (`scripts/sync-automation.mjs`:
CTRF results → TC fields + execution logs + matrix, surgical YAML
mutation). 19 skills total. Tests: 95 → 160+ across 10 files.
