...
2. **Name the test correctly.** First line of the test name is
   the TC-ID, then a short summary. This is the discipline that
   keeps the brain and the automation repo linked. See
   `playbooks/automation-flow.md`
...
test.describe('<TICKET-KEY> — <area>', () => {
  ...
  test('TC-<TICKET-KEY>-<NN> — <one-line summary from TC title>', async ({ page /* or apiClient */ }) => {
